...
function Loaded()
{
	...
	for (y = 0; y < size; y++)
	{
		table.innerHTML += '<tr class="row ml-1"></tr>';
		for (x = 0;x < size; x++)
		{
			var div = '<div class="square border border-dark" onClick="Click(id)" onMouseOver="MouseOver(id)" onMouseOut="MouseOut(id)"></div>';
			row.item(y).innerHTML += '<td class="col p-0 ">' + div + '</td>';
...
function Click(id)
{
	...
	{	console.log(win)
		...
		if (pwin == 0) {
			Swal.fire({
			icon: 'success',
			title: 'Người chơi O thắng',
		  })
		}else
		{
			Swal.fire({
				icon: 'success',
				title: 'Người chơi X thắng',
			  })
			  if(AI==true){
				Swal.fire({
					icon: 'error',
...
function MouseOut(id)
{
	...
	var square = document.getElementsByClassName("square");
	var pos = parseInt(id);
	square.item(pos).style.backgroundColor = "rgb(239, 239, 239)";
}
...
function winHor(x,y,Board)
{
	...
	console.log(`count:${count},countO:${countO}`)
	if (x > 0)
	{
